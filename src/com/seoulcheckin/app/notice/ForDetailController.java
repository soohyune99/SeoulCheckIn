...
public class ForDetailController implements Execute {
	
	// 고객센터 외국인 결과 페이지 컨트롤러
	...
	public Result execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 메소드 결과를 담아줌 
		faqVO = faqDAO.selectForDetail(faqNumber);
		
		// 사이드 부분 외국인 제목 담아줌
		selectForTitles = faqDAO.selectForTitle();
		
		// 이동한 페이지에서 key,val 넘김
		req.setAttribute("faqForDetail", faqVO);
		req.setAttribute("selForTitles", selectForTitles);
		
		//이동할 페이지 경로
		result.setPath("/app/faq/support_section3_for.jsp");
